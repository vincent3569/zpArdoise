...
<head>
	...
	if (($_zp_gallery_page == 'news.php') && (is_NewsArticle())) {echo ' | ' . gettext('News') . ' | ' . getBareNewsTitle(); }
	...
	//<![CDATA[
		...
		<?php if ($_zp_gallery_page == 'image.php') { ?>
			<?php if (hasNextImage()) { ?>var nextURL = "<?php echo getNextImageURL(); $NextURL = true; ?>";<?php } ?>
			<?php if (hasPrevImage()) { ?>var prevURL = "<?php echo getPrevImageURL(); $PrevURL = true; ?>";<?php } ?>
		<?php } else { ?>
...
<div id="page">
	...
	<div id="site-title" class="clearfix">
		...
		<!-- banniere -->
		<div id="banniere">
			<a href="<?php echo html_encode(getSiteHomeURL()); ?>" title="<?php echo gettext('Home'); ?>"><img id="zplogo" src="<?php echo $_zp_themeroot; ?>/images/<?php echo getOption('use_image_logo_filename'); ?>" alt="<?php echo getGalleryTitle(); ?>" /></a>
		</div>
	</div>
	<?php } ?>
	...
	<div id="main-menu">
		...
		<?php } ?>
		<?php if ($_zenpage_and_pages_enabled) { printPageMenu('list-top', '', 'active', '', '', '', 0, false); } ?>
